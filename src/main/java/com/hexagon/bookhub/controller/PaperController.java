...
public class PaperController {
    ...
    public ResponseEntity<?> savePaper(HttpServletRequest request,@RequestBody PaperRequest paperRequest){
    ...
    public ResponseEntity<?> getPapers(){
        log.info("Inside the getPapers in Paper Controller");
        return paperSevice.getPapers();
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<?> editPaper(@PathVariable String id, @RequestBody PaperRequest paperRequest){
        log.info("Inside the editPaper in Paper Controller");
        Paper paper = new Paper(paperRequest.getSubject(), paperRequest.getGrade(), paperRequest.getSchool(), paperRequest.getTerm(), paperRequest.getPaperUrl());
        switch (paperRequest.getPaperType()){
            case "ADVANCED_LEVEL":
                paper.setPaperType(EPaperType.ADVANCED_LEVEL);
        ...
        return paperSevice.editPaper(id,paper);
    }
}
